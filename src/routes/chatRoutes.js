...
// GET /api/chats - 모든 대화 목록 조회 (기본 사용자)
router.get('/', async (req, res) => {
  try {
    // 임시: 기본 사용자 ID 3 사용 (마이그레이션에서 생성된 기본 사용자)
    const defaultUserId = 3;
    const chats = await getAllChats(defaultUserId);
    res.json(chats);
  } catch (error) {
...
// POST /api/chats - 새 대화 생성 (기본 사용자)
router.post('/', async (req, res) => {
  try {
    // 임시: 기본 사용자 ID 3 사용
    const defaultUserId = 3;
    const newChat = await createNewChat(defaultUserId);
...
// GET /api/chats/:chatId - 특정 대화 조회 (기본 사용자)
router.get('/:chatId', async (req, res) => {
  try {
    const { chatId } = req.params;
    // 임시: 기본 사용자 ID 3 사용
    const defaultUserId = 3;
    const chat = await getChatById(chatId, defaultUserId);
    res.json(chat);
  } catch (error) {
...
router.post('/:chatId/messages', async (req, res) => {
  try {
    ...
    console.log('📊 Total contexts found:', conversationContexts.length);
     ...
     try {
       // 통합된 프롬프트 생성
       const integratedPrompt = generatePrompt(message, conversationContexts);
